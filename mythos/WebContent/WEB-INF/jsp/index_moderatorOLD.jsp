								  ...
								  <c:if test='${product.flagType!=4}'>
								      ...
								      <td>
								     	<c:choose>
								      	...
								      	<select id='location<c:out value="${product.getIdProduct()}"/>' class="form-control" style='width:auto; margin:auto;' onchange="modFlag(<c:out value="${product.getIdProduct()}"/>)">
...
<script>
	...
	function modFlag(id){
		...
		$.get("EditProduct?id=" + id+"&location="+location+"&flag=location", 
				function(data){
					updatePage();
					$("#productsSection").show();
					goToRow(id);

					//$("#msg").text("Locazione modificata!");
					//OpenAlert();
		});
	}

	function modWareHouse(id){
		...
		$.get("EditProduct?id=" + id+"&warehouse="+warehouse+"&flag=warehouse", 
				function(data){
					//updatePage();
					$("#productsSection").show();
					
					$("#msg").text("Quantità magazzino modificata!");
					OpenAlert();	
				
		});
	}
	
	function plusGalley(id){
		
		var galley=$('#addQtyG'+id).val();
		
		if(galley.trim().length==0){
			$("#msg").text("La quantità non può essere nulla!");
			OpenAlert();	
			return;
		}

		$.get("EditProduct?id=" + id+"&galley="+galley+"&flag=galley_plus", 
				function(data){
					updatePage();
					$("#productsSection").show();

					goToRow(id);
					$("#msg").text("Quantità cambusa modificata!");
					OpenAlert();
		});	
	}
	...
	function plusBar(id){
		...
		$.get("EditProduct?id=" + id+"&bar="+bar+"&flag=bar_plus", 
				function(data){
					updatePage();
					$("#productsSection").show();
					
					goToRow(id);
					$("#msg").text("Quantità bar modificata!");
					OpenAlert();
		});	
	}
	...
	function modPrice(id){
		...
		$.get("EditProduct?id=" + id+"&price="+price+"&flag=price", 
				function(data){
					//updatePage();
					$("#productsSection").show();
					$("#msg").text("Prezzo modificato!");
					OpenAlert();
		});
